...
data$DinoBac <- data$Dinophyceae/data$Bacillariophyceae
data_trend <- data 
...
datab <- dplyr::select(data,Code_point_Libelle, Date,CHLOROA:Ciliophora,N_noeuds:cluster,DinoBac)

# Correlations between metrics and abiotic environment
datac <- dplyr::select(datab,SALI:`TURB-FNU`,N_noeuds,D_liens,C_tance,Avg_p_length,Adhes,Mod,Assort,Nat_connect,DinoBac)
datac$DinoBac[datac$DinoBac == "Inf"] <- NA
ncomp <- estim_ncpPCA(datac)
res.imp <- imputePCA(datac, ncp = ncomp$ncp,method = "EM")
datac <- res.imp$completeObs
r <- cor(datac)
...
for (k in c(38)){
  data <- dplyr::select(datab,Code_point_Libelle,Date,colnames(datab)[k])
  data <- data[complete.cases(data$DinoBac),]
  data$DinoBac[data$DinoBac == "Inf"] <- NA
  var <- colnames(datab)[k]
  for (i in c(1:19)){
    station <- levels(as.factor(data$Code_point_Libelle))[i]
    Table <- filter(data, Code_point_Libelle == station)
...
datab <- bind_rows(data1,data2,data3)
...
data <- read_delim("data_modif/Tendances_tau.csv", 
                   delim = ";", escape_double = FALSE, locale = locale(decimal_mark = ",", 
                                                                       grouping_mark = ""), trim_ws = TRUE)
data <- t(data[1:19,-c(1,20:22)])
distdata <- dist(data,method = "euclidean")
clust <- hclust(distdata,"complete")
plot(clust,main=NULL,xlab="Parametre",ylab="Distance")
