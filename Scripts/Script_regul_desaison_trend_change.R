...
outliers <- function(x) {
  ...
  indexes <- which(median_dev > 5.2 * mad(x,na.rm = T))
  return(indexes)
}

# REGULARISE 1:7,9:17,19:21)

for (i in c(1:7,9:17,19:21)){
  ...
  t_reg <- regul(x=Table$Date, y=Table$CHLOROA, xmin=newtmin, n=new_n, deltat=newinterval,method="constant")
  ...
  outliers_CHLOROA<- outliers(Table_regul$CHLOROA)
  #windows(title = "Outlyers")
  plot(y=Table_regul$CHLOROA,x=Table_regul$Date, type="l",main = paste0("Outliers pour ",station))
  points(
    # the x coordinate is the index
    x=Table_regul[outliers_CHLOROA,]$Date,
    # the y coordinate is the value
    y=Table_regul$CHLOROA[outliers_CHLOROA],
    # make points solid and red
    pch=16, col="red")
  nom_fichier <- paste0("Outliers_regularise",station)
  ...
  write.csv2(Table_regul,file=paste0("data_outliers/",nom_fichier,".csv"), row.names = FALSE,dec = ".")
}
...
for (i in c(1:7,9:17,19:21)){
  station <- levels(as.factor(data$Code_point_Libelle))[i]
  Table <- filter(data, Code_point_Libelle == station)
  outliers_CHLOROA<- outliers(Table$CHLOROA)
  #windows(title = "Outlyers")
  plot(y=Table$CHLOROA,x=Table$Date, type="l",main = paste0("Outliers pour ",station))
  points(
    # the x coordinate is the index
    x=Table[outliers_CHLOROA,]$Date,
    # the y coordinate is the value
    y=Table$CHLOROA[outliers_CHLOROA],
    # make points solid and red
    pch=16, col="red")
  nom_fichier <- paste0("Outliers_brut",station)
  nom_fichier <- paste0(nom_fichier)
  Table_regul$Code_point_Libelle <- station
  Table_regul$Outlier <- "NON"
  Table_regul[outliers_CHLOROA,]$Outlier <- "OUI"
...
for (i in c(1:21)){
  ...
  plot(dec, scales=list(y="free"))
  Table_regul$CHLOROA_noseason <- dec$data$raw - dec$data$seasonal
  Table_regul$CHLOROA_trend <- dec$data$trend
  Table_regul$CHLOROA_season <- dec$data$seasonal
  
  tendtest <- mk.test(dec$data$trend)
  cat("Tendance de",tendtest[["estimates"]][["tau"]],"pour ",station,"
      ")
  residual <- dec$data$remainder
  #plot(acf(residual,na.action = na.pass,plot=F),main = paste0("Structure residu pour ",station))
  ...
  # # make points solid and red
  # pch=16, col="red")
  nom_fichier <- paste0("Outliers_regularise_desaisonalise_Vtrend",station)
  nom_fichier <- paste0(nom_fichier)
  Table_regul$Code_point_Libelle <- station
  Table_regul$Outlier <- "NON"
  Table_regul[outliers_CHLOROA,]$Outlier <- "OUI"
  ...
  Table_regul$pvaltrend <- tendtest[["p.value"]]
  Table_regul$pvalchange <- changetest$p.value
  Sensslope <- sens.slope(dec$data$trend)
  Table_regul$psens <- Sensslope$p.value
  Table_regul$slopesens <- as.numeric(Sensslope$estimates)
