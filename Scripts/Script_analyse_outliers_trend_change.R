...
# Making the graph to visualize at has been done for all the stations
...
# The same but manually to find the shift date
# Reference date
datagraph <- data_Toulon
date_reference <- datagraph$Date[complete.cases( datagraph$Changepoint)]

# Find the closiest date
...
# That's ok now
write.csv2(data_withoutliers_ok,file="data_modif/Table_FLORTOT_Surf_0722_COM_period_Stselect_hydro_phyto_chloro_phylum_period15_chlafilter_cluster5_div_withoutliers_final.csv", row.names = FALSE,dec = ".")


### Comparison of detection methods for each station ####
{
  Q95 <- quantile(data_Calvi$CHLOROA_noseason, 0.95,na.rm = T) # Quantile 95
Q90 <- quantile(data_Calvi$CHLOROA_noseason, 0.90,na.rm = T) # Quantile 90
Q99 <- quantile(data_Calvi$CHLOROA_noseason, 0.99,na.rm = T) # Quantile 99
MAD <- min(filter(data_Calvi,Outlier == "OUI" & CHLOROA_noseason >= 0)$CHLOROA_noseason) #MAD
nrow(filter(data_Calvi,CHLOROA_noseason >= Q90))  # Number of bloom by methods
nrow(filter(data_Calvi,CHLOROA_noseason >= Q95))  # Number of bloom by methods
nrow(filter(data_Calvi,CHLOROA_noseason >= Q99))  # Number of bloom by methods
nrow(filter(data_Calvi,CHLOROA_noseason >= MAD))  # Number of bloom by methods
nrow(data_Calvi) # Number of dates
